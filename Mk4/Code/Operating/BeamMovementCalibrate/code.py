...
def PrepareThenPerformScan():
    ...
    for i in range(10):
        ...
        print("Loop %u of 10" %(i))

        BeltTakeStepperPower()
        # Move until beam is broken

        print("Move until reach object boundary")

        foundBoundary = False
    

        while not foundBoundary:
            MoveBeamInX(1)
            #print("Position %u Beam broken: %s" %(currentX(), ["broken", "unBroken"][BeamIsBroken()]))

            # Moving from bream not broken to beam broken
            if BeamIsBroken():
                print("Beam broken at %u" %(currentX()))
                # double check
                time.sleep(delay)
                if BeamIsBroken():
                    print("Confirm Beam broken at %u" %(currentX()))
                    time.sleep(delay)
                    print("Moving back towards home position to check boundary is crossed again")
                    MoveBeamInX(-threshold)
                    time.sleep(delay)
                    unBroken = not BeamIsBroken()
                    print("Retesting at {0} - {1}".format(currentX(),["object still detected", "object not detected"][unBroken]))
                    MoveBeamInX(+2*threshold)
                    time.sleep(delay)
                    reBroken = BeamIsBroken()
                    print("Retesting at {0} - {1}".format(currentX(),["object no longer detected", "detected object"][reBroken]))
                    
                    if unBroken and reBroken:
                        objectStart = currentX()
                        print("Confirmed boundary at %u" %(currentX()))
                        time.sleep(delay)
                        foundBoundary = True
                        break
                    else:
                        MoveBeamInX(-2*threshold)
                        print("Inconsistent boundary - retreat and resume scan  at %u" %(currentX()))
                        time.sleep(delay)
        # Move until beam is not broken

        while True:
            if isOKPressed():
                break

        print("Move until reach end of object")
        ...
        while not foundBoundary:
            MoveBeamInX(1)
            stepCount += 1
            # Moving from bream broken to beam not broken
            if not BeamIsBroken():
                time.sleep(0.1)
                if not BeamIsBroken():
                    MoveBeamInX(10)
                    ...
                    time.sleep(0.1)
                    unBroken = not BeamIsBroken()
                    if reBroken and unBroken:
                        objectEnd = currentX()
                        print("Beam not broken at %u" %(objectEnd))
                        MoveBeamInX(100)
                        time.sleep(0.2)
                        foundBoundary = True
        # print number of steps
        print("Steps with beam broken %u" %(objectEnd - objectStart))
        # reverse direction
