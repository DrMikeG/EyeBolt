...
# Table
stepperAdvanceFromHomeForSpriteCanFromHome = 50 * 100
...
def MoveTableToScanStartPosition():
    ...
    for _ in range(stepperAdvanceFromHomeForSpriteCanFromHome):
        StepTableDownOneStepWithMinDelay()
        CheckEBreak()
        # This assert cannot be the result of mistiming and should not be re-tried
...
def TakeMeasurement(allowFastforward = True):
    ...
    if not CarriageAtRearStop():
        print("** WARN ** TakeMeasurement() Carriage not at rear stop")
        # PokerHome Takes Power for itself
        ok = PokerHome()
        if not ok:
            print("** ERROR ** TakeMeasurement() Carriage not at rear stop and did not home ok")
...
def ConfirmFirstMeasureToSideOfTable():
    ...
    if (abs(measureAngle-stepsForPokerToTouchTable) <= 10):
        ok = True
